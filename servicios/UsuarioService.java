...
public class UsuarioService {
    ...
    public Optional<Usuario> autenticar(String email, String passwordPlano) {
        ...
        try {
            ...
            if (ADMIN_EMAIL.equalsIgnoreCase(email.trim()) && ADMIN_PASSWORD.equals(passwordPlano)) {
                return Optional.of(new Usuario("Administrador", ADMIN_EMAIL,
                        BCrypt.hashpw(ADMIN_PASSWORD, BCrypt.gensalt(BCRYPT_ROUNDS)), Rol.ADMIN));
            }
            return Optional.empty();
        }
    }
    ...
    public void inicializarAdminDefecto() {
        ...
        Usuario existente = admin.get();
        boolean passwordOk;
        try {
            passwordOk = existente.getPasswordHash() != null && BCrypt.checkpw(ADMIN_PASSWORD, existente.getPasswordHash());
        } catch (IllegalArgumentException e) {
            passwordOk = false;
        }
        ...
        if (!passwordOk) {
            coleccion.updateOne(
                    Filters.eq("email", ADMIN_EMAIL),
                    Updates.combine(
                            Updates.set("nombre", "Administrador"),
                            Updates.set("password_hash", hash),
                            Updates.set("rol", Rol.ADMIN.name())
                    )
            );
            System.out.println("[UsuarioService] Admin por defecto sincronizado → " + ADMIN_EMAIL + " / " + ADMIN_PASSWORD);
        }
    }
}